...
int main() {
    ...
    printf("O resultado da expressão : %.2f\n", result);
...
float evaluateRPN(char *expression) {
    ...
    while (token != NULL) {
        if (isdigit(token[0]) || (token[0] == '-' && isdigit(token[1]))) {
            ...
            // Token é um operador (+, -, *, /,^,seno,cosseno,tangente,raiz quadrada,logaritmo - por serem letras provavelmente tem que usar string compare)
